...
El corpus binario completo fue regenerado el 2026-09-04 mediante `tools/generate-differential-corpus.sh` y ahora está presente en `research/corpus/`. Los artefactos y hashes vigentes se conservan junto al corpus; la tabla original conserva además resultados históricos. El generador usa cabeceras PJL con hora de ejecución, por lo que el SHA-256 del stream completo no es necesariamente determinista aunque el raster y el codec sean iguales.

La regeneración permite repetir el análisis estructural y comparar los bloques raster. Las comparaciones completas deben normalizar o excluir los campos temporales del PJL antes de interpretarse como diferencias de codec.
