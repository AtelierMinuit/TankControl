...
La comparación `diff -u` entre el PPD HPLIP y `hp-smart_tank_500_series_mac.ppd` produjo 508 líneas: la base conserva identificación, `pcl3gui2` y color, sustituye `hpcups` por el filtro propio con ruta absoluta y añade perfiles ICC/icono; también difiere en restricciones de medios. Esto confirma derivación parcial, no equivalencia funcional ni validación de las opciones desbloqueadas.

Hallazgo y corrección: la revisión inicial detectó 178 líneas `UIConstraints` en HPLIP y ninguna en el PPD propio. Se restauraron esas 178 restricciones y se actualizó el test heredado que exigía su ausencia. El conteo actual coincide con HPLIP; siguen pendientes la validación física de las combinaciones y la explicación de las restantes diferencias del PPD. Las opciones no probadas físicamente permanecen experimentales.
