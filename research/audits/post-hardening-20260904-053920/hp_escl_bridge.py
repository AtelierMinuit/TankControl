...
class ESCLBridge:
    ...
    def get_capabilities(self) -> str:
        if not self.mock:
            raise RuntimeError("Capacidades físicas no integradas de forma segura en el bridge")
        return MOCK_SCAN_CAPS_ESCL

    def get_status(self) -> str:
        if not self.mock:
            raise RuntimeError("Estado físico no integrado de forma segura en el bridge")
        jobs_xml = []
        with self.jobs_lock:
