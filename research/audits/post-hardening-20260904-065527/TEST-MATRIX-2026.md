...
Actualización 2026-09-04 06:20: la ejecución vigente terminó en **134/134 PASS**; el cliente HTTP USB rechaza respuestas truncadas y el instalador manual exige una build auditada explícita.

La línea histórica de resultados anterior a esta actualización queda superada: la ejecución vigente es **134/134 PASS**.

Revalidación física 06:22: `hp-smart-tank-tool info` PASS para inventario; `scan-caps` y `scan-status` FAIL controlado por timeout, ambos con código 1. No hubo impresión, escaneo ni mantenimiento.

PPD: `cupstestppd -W all` PASS (código 0) con advertencias documentadas por recursos no instalados y nombres de tamaños no estándar.

Performance offline reproducida: una página CUPS Raster 600 DPI produjo 131.307 bytes PCL3GUI en 0,03 s, con RSS máxima de 5.865.472 bytes; sin hardware.

Desinstalador del payload: `--dry-run` PASS tras expansión del paquete; eliminación real NOT TESTED.

Última ejecución de suite: `python3 -m unittest discover -s tests`: **137/137 PASS**. Incluye 2.000 round-trips property-based Mode10 con seed `20260904`, pruebas eSCL fragmentadas/chunked y regresiones de seguridad de locks USB, temporales privados, salida explícita del scanner, bridge local, confirmación de hardware, propagación de errores LEDM, sanitización de logs, desinstalador por URI/recibo, postinstall sin cambios recursivos a iconos ajenos, ausencia de XML eSCL completo en logs, temporales ICC privados, wrapper, icono, calibración, gemelo digital HTTP y cliente HTTP USB; la suite no sustituye los casos físicos marcados `NOT TESTED` o `FAIL`.
