...
class PrinterManager: ObservableObject {
    ...
    func toolBinaryPath() -> String {
        ...
        return "/usr/local/bin/hp-smart-tank-tool"
    }

    func runToolCommand(_ args: [String]) -> String {
        let bin = toolBinaryPath()
        let task = Process()
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func checkAlerts() {
        if printer.statusCategory == "mediaJam" {
            sendNotification(title: "Atasco de Papel", body: "Se detectó un atasco de papel en la HP Smart Tank 500.")
        } else if printer.statusCategory == "mediaEmpty" {
            sendNotification(title: "Sin Papel", body: "La bandeja de la HP Smart Tank 500 está vacía.")
        } else if printer.statusCategory == "closeDoorOrCover" {
            sendNotification(title: "Cubierta Abierta", body: "La cubierta frontal de la impresora está abierta.")
        }
        for item in printer.supplies {
            if item.level > 0 && item.level <= 12 {
                sendNotification(title: "Nivel de Tinta Bajo", body: "El tanque \(item.colorName) está al \(item.level)%. Por favor recargue el depósito.")
            }
        }
    }
