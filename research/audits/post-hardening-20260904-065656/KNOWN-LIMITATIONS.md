...
- Los dos caminos de instalación preservan una cola CUPS preexistente con el nombre del proyecto: el instalador manual aborta y el `postinstall` no la reconfigura. La instalación limpia aún no se ha ejecutado.
- `install_native.sh --dry-run` valida sin privilegios y devuelve código 2 cuando encuentra la cola existente; la ejecución real queda deliberadamente sin realizar hasta una migración explícita.
...
- Revalidación reciente: `status`, `supplies` y `odometer` siguen terminando con `LIBUSB_ERROR_TIMEOUT` y código 1; no se presentan como lecturas físicas.
