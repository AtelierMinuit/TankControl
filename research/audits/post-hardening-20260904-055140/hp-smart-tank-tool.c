    ...
    while (total_read < max_resp_len - 1) {
        ...
        if (r == LIBUSB_SUCCESS && chunk_read > 0) {
            ...
            if (hdr_end) {
                ...
                } else if (strstr(resp_buf, "Transfer-Encoding: chunked")) {
                    fprintf(stderr, "Transfer-Encoding chunked no soportado de forma segura\n");
                    return -1;
                } else {
            ...
            fprintf(stderr, "Timeout esperando respuesta HTTP\n");
            return -1;
        } else {
            break;
        }
...
static void parse_and_print_supplies(const char *xml) {
    ...
    while ((ptr = strstr(ptr, "ConsumableInfo")) != NULL) {
        ...
        if (c_ptr) sscanf(c_ptr, "ConsumableLabelCode>%15[^<]", code);

        const char *l_ptr = strstr(ptr, "ConsumablePercentageLevelRemaining>");
        if (l_ptr) sscanf(l_ptr, "ConsumablePercentageLevelRemaining>%d", &level);

        const char *s_ptr = strstr(ptr, "ConsumableState>");
        if (s_ptr) sscanf(s_ptr, "ConsumableState>%31[^<]", state);
        ...
        if (code[0] != '\0') {
            ...
            if (level >= 0) {
                ...
                const char *st_color = (strcmp(state, "inSensorRange") == 0 || strcmp(state, "ok") == 0) ? "\033[1;32m" : "\033[1;31m";
        ...
        ptr += 14;
        const char *end_tag = strstr(ptr, "ConsumableInfo>");
        if (!end_tag) break;
        ptr = end_tag + 15;
    }
    if (items == 0) {
...
static void parse_and_print_status(const char *xml) {
    ...
    if (s_ptr) {
        ...
        printf("  Categoría: %s\n", cat);
        if (strcmp(cat, "ready") == 0) printf("  Diagnóstico: \033[1;32m● Impresora lista y en reposo\033[0m\n");
        else if (strcmp(cat, "processing") == 0) printf("  Diagnóstico: \033[1;34m● Imprimiendo o procesando trabajo\033[0m\n");
        else if (strcmp(cat, "closeDoorOrCover") == 0) printf("  Diagnóstico: \033[1;31m▲ Cubierta o puerta abierta\033[0m\n");
        else if (strcmp(cat, "mediaEmpty") == 0) printf("  Diagnóstico: \033[1;33m▲ Sin papel en la bandeja\033[0m\n");
        else if (strcmp(cat, "mediaJam") == 0) printf("  Diagnóstico: \033[1;31m▲ Atasco de papel detectado\033[0m\n");
    } else {
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(cmd, "supplies") == 0) {
        ...
        if (!is_mock && open_channel(ctx, 0xff, 0xcc, 0x00, &chan) == 0) {
            char req[] = "GET /DevMgmt/ProductStatusDyn.xml HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n";
            int bytes = http_request(&chan, req, buffer, sizeof(buffer));
            close_channel(&chan);
            char *ptr = (bytes > 0) ? strstr(buffer, "TotalSpitCount>") : NULL;
            if (!ptr || sscanf(ptr, "TotalSpitCount>%d", &spit_count) != 1) {
