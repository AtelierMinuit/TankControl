        ...
        if (r == 0 && chunk > 0) {
            ...
            if (hdr_end) {
                ...
                if (cl) {
                    ...
                    if (body_bytes >= len) break;
                } else if (strstr(buf, "Transfer-Encoding: chunked")) {
                    fprintf(stderr, "[hp_scan] Transfer-Encoding chunked no soportado de forma segura\n");
                    return -1;
                } else {
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        fprintf(stderr, "Uso: %s <archivo-salida.jpg> [dpi] [modo]\n", argv[0]);
        fprintf(stderr, "Se exige una ruta de salida explícita; no se usa un nombre predecible en /tmp.\n");
        return 2;
    }
    const char *outfile = argv[1];
    int resolution = 300;
    if (argc > 2 && parse_nonnegative_arg(argv[2], &resolution) != 0) {
        fprintf(stderr, "DPI inválido: debe ser un entero entre 75 y 1200\n");
        return 2;
    }
    const char *colormode = (argc > 3 && (strcmp(argv[3], "gray") == 0 || strcmp(argv[3], "Grayscale8") == 0)) ? "Gray" : "Color";

    if (resolution < 75 || resolution > 1200) resolution = 300;

    int max_width = (int)(8.5 * resolution);
    int max_height = (int)(11.69 * resolution); // A4 platen max

    int x_start = 0;
    int y_start = 0;
    int width = max_width;
    int height = max_height;
    if ((argc > 4 && parse_nonnegative_arg(argv[4], &x_start) != 0) ||
        (argc > 5 && parse_nonnegative_arg(argv[5], &y_start) != 0) ||
        (argc > 6 && parse_nonnegative_arg(argv[6], &width) != 0) ||
        (argc > 7 && parse_nonnegative_arg(argv[7], &height) != 0)) {
        fprintf(stderr, "Geometría inválida: use enteros no negativos\n");
        return 2;
    }
    ...
    if (x_start >= max_width) x_start = 0;
    if (y_start >= max_height) y_start = 0;
    if (x_start + width > max_width) width = max_width - x_start;
    if (y_start + height > max_height) height = max_height - y_start;
