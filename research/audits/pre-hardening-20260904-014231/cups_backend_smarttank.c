...
static int acquire_usb_lock(int timeout_sec) {
    int fd = open(USB_LOCK_FILE, O_RDWR | O_CREAT, 0666);
    if (fd < 0) return -1;
...
static void record_job_accounting(const char *job_id, const char *user, const char *title, int pages, size_t bytes, const char *options) {
    ...
    FILE *fp = fopen("/tmp/hp_smarttank_accounting.csv", "a");
    if (fp) {
        fseek(fp, 0, SEEK_END);
        if (ftell(fp) == 0) {
        ...
        fprintf(fp, "\"%s\",%s,\"%s\",\"%s\",%d,%zu,%.3f,%.3f,%.4f,%.3f,%.4f,\"%s\"\n",
                time_str, job_id ? job_id : "0", user ? user : "anonymous",
                title ? title : "Untitled", pages, bytes, ink_k_ml, ink_color_ml, total_cost,
                ink_saved_ml, money_saved_usd, saver_label);
        fclose(fp);
    }

    FILE *fj = fopen("/tmp/hp_smarttank_last_cost.json", "w");
    if (fj) {
        fprintf(fj, "{\n"
                    "  \"timestamp\": \"%s\",\n"
                    "  \"job_id\": \"%s\",\n"
    ...
    if (ink_saved_ml > 0.001) {
        fprintf(stderr, "INFO: [smarttank] Contabilidad: Trabajo #%s - %d paginas, Costo: $%.3f USD | Ahorro InkSaver: %.2f ml ($%.3f USD)\n",
                job_id ? job_id : "0", pages, total_cost, ink_saved_ml, money_saved_usd);
    } else {
        fprintf(stderr, "INFO: [smarttank] Contabilidad: Trabajo #%s - %d paginas, Costo estimado: $%.3f USD (Tinta: $%.3f, Papel: $%.3f)\n",
                job_id ? job_id : "0", pages, total_cost, ink_cost, paper_cost);
    }
}
...
static int do_discovery(void) {
        ...
        for (ssize_t i = 0; i < cnt; i++) {
            ...
            if (libusb_get_device_descriptor(devs[i], &desc) == LIBUSB_SUCCESS) {
                if (desc.idVendor == HP_VID && desc.idProduct == HP_PID) {
                    ...
                    printf("direct smarttank://HP/Smart%%20Tank%%20500%%20series?serial=%s "
    ...
    if (!found && (getenv("HP_SMART_TANK_MOCK") != NULL)) {
        printf("direct smarttank://HP/Smart%%20Tank%%20500%%20series?serial=CN1924S1W7 "
               "\"HP Smart Tank 500 series\" "
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stderr, "DEBUG: [smarttank] Iniciando trabajo #%s ('%s') de %s (copias: %d, opciones: %s)\n",
            job_id, title, user, copies, options);
    ...
    if (filename) {
        ...
        if (input_fd < 0) {
            perror("ERROR: No se pudo abrir el archivo de impresion");
            return CUPS_BACKEND_FAILED;
        }
    }
