...
| [Apple Developer Forums — launchd e IOKit matching](https://developer.apple.com/forums/thread/108927) | consulta 2026-09-04 | DTS de Apple muestra `LaunchEvents` con `com.apple.iokit.matching`, criterios `idVendor`/`idProduct`, `IOProviderClass=IOUSBDevice` e `IOMatchLaunchStream`; también indica que el proceso debe consumir el evento. No demuestra que el plist de este proyecto se dispare en el host actual. | media-alta |
