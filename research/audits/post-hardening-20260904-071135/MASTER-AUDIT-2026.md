...
Actualización de suite: tras las regresiones de servicios, desinstalador y scanner, la ejecución actual terminó en **136/136 PASS** (`/tmp/scanner-doc-tests.log`). Las cifras 134/134 y 135/135 anteriores se conservan únicamente como historial.

Corrección adicional de postinstall: el script distribuido todavía podía aplicar `chown -R` al directorio completo de iconos y forzar un enlace backend existente. Se reemplazó por validación de archivos propios regulares y preservación de destinos ajenos. ShellCheck y la suite actual terminaron correctamente en **137/137 PASS**.

Corrección de preservación CUPS: `install_native.sh` ahora rechaza una cola `HP_Smart_Tank_500` preexistente antes de copiar componentes, y el `postinstall` del paquete la conserva sin reconfigurarla. En el host auditado la cola existente usa `usb://`; no se modificó. ShellCheck y la suite actual terminaron en **139/139 PASS**.

Modo reversible de instalación: `install_native.sh --dry-run` quedó implementado. En la ejecución sobre este host terminó con código 2 porque detectó la cola preexistente y abortó antes de cualquier `sudo`; ese resultado es el bloqueo de preflight esperado y confirma que no se puede sobrescribir la instalación ajena.

Pasada de consistencia 06:53: la build C limpia `research/builds/audit-clean/20260904-065355` terminó con código 0; `clang --analyze` por fuente, ShellCheck de los scripts auditables y la suite completa también terminaron correctamente (**139/139 PASS**). No se instaló el paquete ni se modificó el sistema.

Validación estructural corregida: un primer recorrido que dividía rutas por espacios produjo falsos fallos dentro de `HP Smart Tank Utility.app`. Se repitió con lectura NUL-safe: **303 plists** y **166 scripts** pasaron `plutil -lint`/`bash -n` sin errores.

Revalidación física segura: sobre la build `20260904-065355`, `status`, `supplies` y `odometer` terminaron con código 1 por `LIBUSB_ERROR_TIMEOUT`; no hubo respuesta XML válida. La corrección de interfaz EWS de `waste-ink` queda probada offline, pero no resuelve todavía la sincronización/transporte físico.

Integración eSCL mock reproducida: en `127.0.0.1:18092`, `ScannerCapabilities` y `ScannerStatus` respondieron 200; `POST /eSCL/ScanJobs` respondió 201 y `NextDocument` respondió 200 con JPEG de 22 bytes y marcadores SOI/EOI. Evidencia exclusiva del bridge simulado; la captura física y Captura de Imagen siguen sin demostrarse.

Corrección de configuración eSCL: el valor por defecto de `hp_escl_bridge.py` era 8080, mientras el LaunchAgent usa 8089. Se unificó en 8089 y se añadió regresión; la suite actual terminó en **140/140 PASS**.
...
- El paquete de distribución vigente está invalidado por entradas AppleDouble `._*` detectadas en su BOM; el auditor actualizado rechaza esa condición. No existe todavía un paquete limpio apto para distribución y sigue pendiente la firma/notarización.
- La inspección adicional corrigió referencias heredadas a `research/builds`, `scratch` y rutas de desarrollo en el wrapper y bridge del staging; el staging actual queda sin esas referencias. `launchctl print gui/501/com.hp.smarttank.airscan` confirma que el servicio no está cargado en la sesión auditada: el LaunchAgent queda validado estáticamente, no como integración runtime demostrada.
...
“OEM/Enterprise Grade”, “AirPrint completo”, “ICC calibrado”, “ahorro físico 25/50/75%”, “KGray físico”, “backend bidireccional funcional”, “100% verificado” y “opciones desbloqueadas” no quedan demostradas por código, mock o `sips`. La ficha oficial de HP consultada indica USB Only/Wireless No para la serie, pero contiene una nota genérica de AirPrint; la conclusión defendible es `AIRPRINT: NO IMPLEMENTADO`, no incompatibilidad absoluta del modelo.
...
Nota de actualización: las ejecuciones 131/131, 133/133, 134/134 y 139/139 son históricas; la suite vigente registrada en esta pasada es **140/140 PASS**.
...
Nota de consistencia: las cifras 134/134 y la build `061000` citadas en el párrafo histórico anterior no representan el estado vigente; la ejecución actual es **140/140 PASS** sobre el estado de fuentes actual. La build C de referencia documentada sigue siendo `research/builds/audit-clean/20260904-065355`.
...
El decoder offline pasó 9/9 pruebas y Mode10 es el camino activo observado; Mode9 no debe documentarse como activo. La revisión regeneró un caso diferencial mínimo: HPLIP produjo 6.996 bytes y el filtro nativo 7.840 bytes desde el mismo raster. El comparador encontró diferencias estructurales y el decoder aceptó ambos streams; esto es evidencia offline reproducida, no equivalencia byte a byte ni validación física.
...
La referencia primaria de Apple confirma que `com.apple.iokit.matching` puede usar criterios USB de vendor/product y `IOMatchLaunchStream`, pero también indica que el proceso debe consumir el evento. Esa fuente respalda la forma general del mecanismo, no la activación efectiva de este LaunchAgent en el host auditado.
...
Corrección semántica del filtro: los comentarios de `rastertopcl3gui.c` ahora describen atenuación raster nominal y no afirman volumen físico de tinta ni control exclusivo de inyectores. La build `research/builds/audit-clean/20260904-065851` compiló correctamente; la suite actual mantiene **140/140 PASS**.
