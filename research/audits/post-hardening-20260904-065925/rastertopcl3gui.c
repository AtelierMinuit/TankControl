...
int main(int argc, char *argv[]) {
    ...
    while (cupsRasterReadHeader2(ras, &header)) {
        ...
        if (!is_gray && !is_cmyk && header.cupsBitsPerPixel != 24) {
            fprintf(stderr, "ERROR: [rastertopcl3gui] Profundidad de color no soportada (%d bpp, colorspace %d). Se requiere 8-bit Gray, 24-bit sRGB o 32-bit CMYK.\n",
        ...
        /* CRD sRGB Mode 10 */
        fwrite(crd_color_mode10, 1, sizeof(crd_color_mode10), stdout);
