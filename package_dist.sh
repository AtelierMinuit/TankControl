...
cp -X "${DIR}/tools/hp_airprint_daemon.sh" "${PKG_ROOT}/usr/local/share/hp-smart-tank/"
cp -X "${DIR}/tools/hp_ipp_submit.sh" "${PKG_ROOT}/usr/local/share/hp-smart-tank/"
cp -X "${DIR}/tools/hp-smart-tank-500-airprint.conf" "${PKG_ROOT}/usr/local/share/hp-smart-tank/"
...
# AirPrint se ejecuta como servicio IPP Everywhere separado. A diferencia del
# bridge eSCL local, este servicio escucha en la LAN de forma intencional.
cat << 'PLIST' > "${PKG_ROOT}/Library/LaunchAgents/com.hp.smarttank.airprint.plist"
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.hp.smarttank.airprint</string>
    <key>ProgramArguments</key>
    <array>
        <string>/usr/local/share/hp-smart-tank/hp_airprint_daemon.sh</string>
        <string>--port</string>
        <string>8631</string>
        <string>--queue</string>
        <string>HP_Smart_Tank_500</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
...
if [ -n "$CURRENT_USER" ] && [ "$CURRENT_USER" != "root" ]; then
    ...
    for agent in com.hp.smarttank.airscan com.hp.smarttank.airprint; do
        if ! launchctl asuser "$USER_ID" launchctl load -w "/Library/LaunchAgents/${agent}.plist" 2>/dev/null; then
            echo "ADVERTENCIA: no se pudo cargar ${agent}; los archivos quedaron instalados pero el servicio no está activo."
        fi
    done
fi

echo "Archivos del controlador configurados; verifique por separado la cola CUPS, AirScan y AirPrint."
exit 0
POSTINSTALL
...
if xattr -lr "${PKG_ROOT}" "${PKG_SCRIPTS}" 2>/dev/null | grep -v 'com.apple.provenance' | grep -q .; then
